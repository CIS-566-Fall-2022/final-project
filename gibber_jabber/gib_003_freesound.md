can use scrape internet for sounds on [freesound](https://freesound.org/people/RealRhodesSounds/sounds/4048/) site
...
// the number of sou ds in the instrument is stored in .length
...
d = Freesound({ 
  ...
  maxVoices:2							// ... but only play one at a time
})
d.pick.seq( Rndi(0,d.length), 2 )
d.trigger.seq( 2,[1, 1/2, 1/2, 1] ) //can have a list of timings []
