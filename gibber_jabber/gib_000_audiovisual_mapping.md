text
technically 4, i misnumberd some rip
```
...
sphere = Sphere().render()
kick = Kick().trigger.seq( [1, 3, 1, 2],[1/4, 1/8, 1/8, 1/64, 1/32, 1/16, 1/8] ) //they [] will link up if same size
sphere.radius.map( kick, 2, 0.5 )
...
Fog( .5, Vec3(0))
rpt = Repeat( Sphere(.1), .2 ).render()
syn = PolySynth('rhodes')
rpt.distance.x.map( syn, 10, 0 )
syn.chord.seq( [[0,2,4,5],[0,2,4,7] ], [1/4, 1/8, 1/8, 1/64, 1/32, 1/16, 1/8] ) /chord seq in diff timings
