...
// of a cycle
drums.tidal( 'kd sd kd sd' ) // what if i added a violin sound to this?
...
// technique:
	// i'm using <> for alternating options
	// i'm using *n to make n-lets
	// i'm using [] to make groups
	// /d multiplied the part of the cycle into d times longer
				// ed: sd sd = sd/2


// let's add some funky drums that (ab)use the notation above
drums = Drums()
drums.tidal( '[<kd kd*2 kd *6> 	<ch*4 sd*7 oh sd*3>] sd [sd/2] <kd sd*2> oh*3' )
// ^ in 7: 		1                    2.                       3.   4. 5.   6.         7
// i give up bc i cant count lol


// now let's add a euclidian rhythm to go along 
// also in 7
// i like it in 10 more
d2 = Drums()
d2.tidal('oh(6,10)')

// let's add a synth
verb = Bus2('reverb')
syn = PolySynth('bleep.echo').connect(verb, 0.4)
syn.note.tidal('0 1 2 3 [5 6 <13 12 [9 17 29 31]>]*4')


Clock.bpm = Math.sin(time) * 120
